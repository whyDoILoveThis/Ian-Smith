...
import { dateToX, xToDateMs } from "@/components/0Timeline/hooks/useTimelineMath";
import { BASE_RANGE_DAYS, MS_PER_DAY } from "@/components/0Timeline/lib/constants";
...
const DEMO_NODES: TimelineNode[] = [
  ...
  { nodeId: "demo-1", timelineId: "demo", title: "Research Phase", description: "Market research and feasibility study conducted across 3 target segments", dateMs: new Date("2021-03-10").getTime(), images: [{ url: "https://picsum.photos/seed/research/600/340" }], color: "#14b8a6" },
  { nodeId: "demo-2", timelineId: "demo", title: "Initial Concept", description: "First wireframes and concept docs created in Figma", dateMs: new Date("2021-04-02").getTime(), images: [{ url: "https://picsum.photos/seed/wireframe/600/340" }], color: "#06b6d4" },
  { nodeId: "demo-3", timelineId: "demo", title: "Team Formed", description: "Core contributors assembled — 4 engineers, 1 designer", dateMs: new Date("2021-04-28").getTime(), images: [], color: "#0ea5e9" },

  // --- spread out individual nodes ---
  { nodeId: "demo-4", timelineId: "demo", title: "First Prototype", description: "MVP delivered with core features including drag-and-drop and real-time sync", dateMs: new Date("2022-01-15").getTime(), images: [{ url: "https://picsum.photos/seed/prototype/600/340" }, { url: "https://picsum.photos/seed/mvp/600/340" }], color: "#22c55e" },
  { nodeId: "demo-5", timelineId: "demo", title: "Beta Launch", description: "Public beta with early adopters — 200+ users in first week", dateMs: new Date("2022-11-08").getTime(), images: [{ url: "https://picsum.photos/seed/betalaunch/600/340" }], color: "#8b5cf6" },

  // --- mid cluster (close dates → cluster) ---
  { nodeId: "demo-6", timelineId: "demo", title: "Security Audit", description: "Third-party security review completed with zero critical findings", dateMs: new Date("2023-06-01").getTime(), images: [], color: "#ef4444" },
  { nodeId: "demo-7", timelineId: "demo", title: "Performance Pass", description: "Major perf optimizations shipped — 60% faster initial load", dateMs: new Date("2023-06-18").getTime(), images: [{ url: "https://picsum.photos/seed/perfchart/600/340" }], color: "#f97316" },

  // --- spread out ---
  { nodeId: "demo-8", timelineId: "demo", title: "AI Integration", description: "AI-powered timeline generation added — describe any topic and get a full timeline in seconds", dateMs: new Date("2024-02-12").getTime(), images: [{ url: "https://picsum.photos/seed/aifeature/600/340" }, { url: "https://picsum.photos/seed/aiui/600/340" }], color: "#f59e0b" },
  { nodeId: "demo-9", timelineId: "demo", title: "Public Release", description: "Version 1.0 shipped to production with full documentation", dateMs: new Date("2024-09-20").getTime(), images: [{ url: "https://picsum.photos/seed/v1launch/600/340" }], color: "#ec4899" },

  // --- late cluster ---
  { nodeId: "demo-10", timelineId: "demo", title: "Multi-User Support", description: "Real-time collaboration added — multiple users can edit simultaneously", dateMs: new Date("2025-05-05").getTime(), images: [{ url: "https://picsum.photos/seed/collab/600/340" }], color: "#3b82f6" },
  { nodeId: "demo-11", timelineId: "demo", title: "V2 Shipped", description: "Major update with new UI, dark mode, and keyboard shortcuts", dateMs: new Date("2025-05-22").getTime(), images: [], color: "#6366f1" },
  { nodeId: "demo-12", timelineId: "demo", title: "Mobile App", description: "Native mobile companion launched on iOS and Android", dateMs: new Date("2025-06-10").getTime(), images: [{ url: "https://picsum.photos/seed/mobileapp/600/340" }], color: "#a855f7" },
];
...
function FeatureBadge({ icon, label, delay }: { icon: string; label: string; delay: number }) {
  return (
    <motion.div
...
function useFloatingParticles(count = 14) {
  const [particles, setParticles] = useState<
    { id: number; x: number; delay: number; duration: number; size: number; opacity: number }[]
  >([]);
...
export default function TimelineShowcase() {
  ...
  const getXForMs = useCallback(
    (ms: number) => dateToX(ms, centerMs, scale, containerWidth, BASE_RANGE_DAYS),
    [centerMs, scale, containerWidth]
  );
  const getMsForX = useCallback(
    (x: number) => xToDateMs(x, centerMs, scale, containerWidth, BASE_RANGE_DAYS),
    [centerMs, scale, containerWidth]
  );
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (target.closest(".overflow-y-auto, .overflow-auto, .customScroll")) return;
      ...
      if (e.altKey) {
        ...
        const startAfter = beforeMs - (mouseLocalX / containerWidth) * visibleMsAfter;
      ...
      <div className="absolute inset-0 -z-5 overflow-hidden rounded-3xl pointer-events-none">
        {particles.map((p) => (
          <motion.div
            ...
            animate={{ y: [0, -600], opacity: [0, p.opacity, p.opacity * 0.8, 0] }}
        ...
        <div className="p-8 md:p-12 lg:p-16">
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 mb-8">
            <div className="max-w-md flex-shrink-0">
              ...
              >
                ...
                <span className="text-white/70 font-semibold">pointer-lock panning</span>,
                zoom, drag &amp; drop events, and{" "}
                <span className="text-white/70 font-semibold">AI-powered timeline generation</span>.
                Create, share, and explore timelines with beautiful hover cards and node clustering.
              </motion.p>

              <div className="flex flex-wrap gap-2 mt-6">
                <FeatureBadge icon={"\uD83D\uDDB1\uFE0F"} label="Pan & Zoom" delay={0.35} />
                <FeatureBadge icon={"\u2728"} label="AI Generation" delay={0.4} />
                <FeatureBadge icon={"\uD83D\uDC65"} label="Multi-User" delay={0.45} />
                <FeatureBadge icon={"\uD83D\uDDBC\uFE0F"} label="Image Nodes" delay={0.5} />
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="border-b border-white/[0.06] demo-tl-header">
              <div className="relative">
                ...
                <style dangerouslySetInnerHTML={{ __html: `
                  .demo-tl-header div.flex.justify-end.items-center.gap-2.w-full > span.whitespace-nowrap {
            ...
            >
              ...
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 pointer-events-none">
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/60 border border-white/10 text-[11px] text-neutral-300 backdrop-blur-sm">
                  <span className="flex items-center gap-1">
                    Hold <kbd className="px-1.5 py-0.5 rounded bg-white/15 text-white font-mono text-[10px] border border-white/20">Alt</kbd>
                    + Scroll to zoom
                  </span>
              ...
              <Toast visible={showToast} message="Explore Timeline for all features" />
            </div>
          </motion.div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
            ...
            >
              Built with React, Framer Motion &amp; Pointer Lock API &mdash; real-time persistence via Appwrite
            </motion.p>
